...
function PremiumDropdown({
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-50 max-h-72 overflow-y-auto rounded-2xl border border-[#EAE7DF] bg-white/95 p-2 shadow-[0_22px_55px_rgba(17,17,17,0.16)] backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200">
          <button
            type="button"
...
export function CatalogPage({
      ...
      const options = Array.from(
        new Set(
          products
            ...
            .filter((tag: string) => tag.startsWith(group.prefix))
        )
      );
  ...
  const headingTitle = searchQuery ? `Search results for "${searchQuery}"` : title;
  const headingDescription = searchQuery
            ...
            <div className="grid grid-cols-1 gap-7 sm:grid-cols-2 lg:grid-cols-4">
              {paginatedProducts.map((product) => (
                <a
                  ...
                  key={product.id}
                  className="group relative block overflow-hidden rounded-3xl border border-[#EAE7DF] bg-white/85 shadow-[0_10px_30px_rgba(17,17,17,0.04)] backdrop-blur-sm transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-[0_22px_55px_rgba(15,90,70,0.14)]"
                >
                  ...
                  <div className="relative h-64 overflow-hidden bg-[#F8F7F3] md:h-72">
                    ...
                    <div className="absolute inset-x-4 bottom-4 translate-y-4 opacity-0 transition-all duration-300 ease-out group-hover:translate-y-0 group-hover:opacity-100">
                      <Button
                        ...
                        className="w-full rounded-2xl bg-[#0F5A46] text-white shadow-[0_12px_28px_rgba(15,90,70,0.30)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#126B54]"
                      >
